...
const buildExpiryTableRows = (items, expiryColor = "#e53e3e") =>
  items
    .map(
      (i) => `
      <tr>
        ...
        <td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;color:${expiryColor};font-weight:600;">
          ${new Date(i.expiry_date).toLocaleDateString("vi-VN")}
        </td>
        <td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;text-align:center;">${escapeHtml(i.quantity)}</td>
        <td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;">${escapeHtml(i.position || i.warehouse_name || "-")}</td>
      </tr>`
    )
    .join("");

// items: [{name, batch_code, expiry_date, quantity, position}]
exports.sendExpiredMedicineAlert = async (to, items) => {
  const dateStr = new Date().toLocaleDateString("vi-VN");
  const tableRows = buildExpiryTableRows(items, "#b91c1c");
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:760px;margin:auto;">
      <div style="background:#991b1b;color:white;padding:20px 24px;border-radius:8px 8px 0 0;">
        <h2 style="margin:0;font-size:18px;">Canh bao: Lo thuoc da het han</h2>
        <p style="margin:4px 0 0;font-size:13px;opacity:.85;">Ngay ${dateStr}</p>
      </div>
      <div style="padding:20px 24px;background:#fef2f2;border:1px solid #fecaca;border-top:none;">
        <p style="color:#7f1d1d;font-size:14px;">
          Cac lo thuoc sau day <strong>da het han su dung</strong>.
          Vui long cach ly khoi khu vuc cap phat va xu ly theo quy trinh.
        </p>
        <table style="width:100%;border-collapse:collapse;background:white;border-radius:6px;overflow:hidden;border:1px solid #e2e8f0;">
          <thead>
            <tr style="background:#fee2e2;">
              <th style="padding:10px 12px;text-align:left;font-size:12px;color:#7f1d1d;font-weight:600;">Ten thuoc</th>
              <th style="padding:10px 12px;text-align:left;font-size:12px;color:#7f1d1d;font-weight:600;">Ma lo</th>
              <th style="padding:10px 12px;text-align:left;font-size:12px;color:#7f1d1d;font-weight:600;">Han su dung</th>
              <th style="padding:10px 12px;text-align:center;font-size:12px;color:#7f1d1d;font-weight:600;">So luong</th>
              <th style="padding:10px 12px;text-align:left;font-size:12px;color:#7f1d1d;font-weight:600;">Vi tri</th>
            </tr>
          </thead>
          <tbody>${tableRows}</tbody>
        </table>
        <p style="margin-top:16px;font-size:12px;color:#991b1b;">
          Email nay duoc gui tu dong tu he thong Pharm WMS luc 07:00 sang moi ngay.
        </p>
      </div>
    </div>
  `;
  ...
  await sendMail({
    to,
    subject: `[Canh bao] Lo thuoc da het han - ${dateStr}`,
    html,
  });
};
    ...
    <div style="font-family:Arial,sans-serif;max-width:760px;margin:auto;">
      ...
      <div style="padding:20px 24px;background:#fffbeb;border:1px solid #fde68a;border-top:none;">
        <p style="color:#78350f;font-size:14px;">
          Cac lo thuoc sau day se het han trong vong <strong>${nearExpiryMonths} thang toi</strong>.
        ...
        <table style="width:100%;border-collapse:collapse;background:white;border-radius:6px;overflow:hidden;border:1px solid #e2e8f0;">
          <thead>
            <tr style="background:#fef3c7;">
              ...
              <th style="padding:10px 12px;text-align:left;font-size:12px;color:#92400e;font-weight:600;">Ma lo</th>
              <th style="padding:10px 12px;text-align:left;font-size:12px;color:#92400e;font-weight:600;">Han su dung</th>
              <th style="padding:10px 12px;text-align:center;font-size:12px;color:#92400e;font-weight:600;">So luong</th>
              <th style="padding:10px 12px;text-align:left;font-size:12px;color:#92400e;font-weight:600;">Vi tri</th>
            </tr>
          </thead>
          <tbody>${tableRows}</tbody>
        </table>
